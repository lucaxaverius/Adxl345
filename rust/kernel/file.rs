...
impl File {
    ...
    /* Updated by Luca Saverio Esposito */
    
    /// Sets the file as non-seekable by calling the underlying `nonseekable_open`.
    ///
    /// # Safety
    /// This function modifies internal file flags to mark it as non-seekable.
    /// It should be used only for character devices that do not support seeking.
    pub fn set_nonseekable(&self) -> Result<()> {
        // SAFETY: Accessing the underlying `bindings::file` and retrieving the inode pointer.
        // In a real use case, ensure that `f_inode` is valid and accessible.
        let file_ptr = self.0.get();
        let inode = unsafe { (*file_ptr).f_inode };

        // Call the C `nonseekable_open` function.
        let ret = unsafe { bindings::nonseekable_open(inode, file_ptr) };
        if ret != 0 {
